...
describe('getCurrentTraceId() under concurrent trace() calls (MetaMask-planning#7523)', () => {
  ...
  function reportCorrelation(label: string, requestId: string): void {
    const viaA = correlatedRequestId(TRACE_ID_A);
    const viaB = correlatedRequestId(TRACE_ID_B);
    const ambient = Sentry.getCurrentScope().getPropagationContext().traceId;
    const viaAmbient = correlatedRequestId(ambient);
    // eslint-disable-next-line no-console
    console.log(
      `PROBE ${label}\n` +
        `  baggage requestId on A's own fetch = ${requestId}\n` +
        `  correlated via A's trace id (${TRACE_ID_A.slice(0, 8)}) = ${String(viaA)}  -> matchesA=${String(viaA === requestId)}\n` +
        `  correlated via B's trace id (${TRACE_ID_B.slice(0, 8)}) = ${String(viaB)}  -> matchesB=${String(viaB === requestId)}\n` +
        `  ambient scope trace id = ${ambient} -> correlated = ${String(viaAmbient)} matchesAmbient=${String(viaAmbient === requestId)}`,
    );
  }
  ...
  describe('when a second, unrelated trace() call is still pending', () => {
    ...
    it('correlates an operation’s own outbound request with its own trace id, not a concurrently-pending unrelated operation’s', async () => {
      ...
      try {
        ...
        reportCorrelation('INTERLEAVED', requestId);
    ...
    it('correlates correctly once the concurrent operation has already resolved (discriminating control)', async () => {
      ...
      const requestId = requestIdFromBaggage(capturedBaggage());
      reportCorrelation('CONTROL (B resolved first)', requestId);
